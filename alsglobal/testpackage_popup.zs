...
tpdoublcliker = new testpackageDoubleClick_Listener();
...
tpakcliker = new testpackageOnSelect_Listener();
...
void populateTestPackages(Div idiv, String iarcode)
{
	...
	if(tp_recs.size() == 0) return;
	newlb.setRows(10);
	newlb.addEventListener("onSelect", tpakcliker);
	ArrayList kabom = new ArrayList();
	for(tpi : tp_recs)
	{
	...
	lbhand.setDoubleClick_ListItems(newlb, tpdoublcliker);
}
...
tp2clikr = new tp2clients_Listener();
...
void showClientToTestPackage(Div idiv)
{
	...
	newlb.setRows(20);
	newlb.addEventListener("onSelect", tp2clikr );
	ArrayList kabom = new ArrayList();
	for(dpi : custrecs)
	{
