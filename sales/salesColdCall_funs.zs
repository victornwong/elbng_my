...
Object[] actconthds =
{
	new listboxHeaderWidthObj("CLD",true,"60px"),
	...
	new listboxHeaderWidthObj("Role",true,"70px"),
};
...
class acticontclk implements org.zkoss.zk.ui.event.EventListener
{
	public void onEvent(Event event) throws UiException
	{
		...
		taskRecord = showTaskActions(actis_holder, "SALES_MARKETING", JN_linkcode(), nextactiondate_lbl, nextactionstr_lbl );
		if(taskRecord == null) // no task/todo -- create 1 for owner
		{
			lbhand.matchListboxItems(assignee_lb, glob_taskowner); // make sure it's the owner
			taskerFunc("asstask_b");
		}
		//listActivities(glob_sel_acticont); do not use anymore - use internaltaskf_v2
		//listTiedQuotations(glob_sel_custname);
	}
}
acticontclkier = new acticontclk();

// itype: 1=list all, 2=list by username, 3=by class
...
*/
	...
	if(itype == 3)
	{
		kk = listbyclass_lb.getSelectedItem().getLabel();
		byusr = "where potential='" + kk + "'";
	}
	sqlstm += byusr;
